...
public class InternalDomain {
    ...
    public void close(int varsMaxSize) throws IOException {
        ...
        s = "import utilities.jshop2.*;" + endl + "import java.util.Collections;\n" +
                "import java.util.LinkedList;" + endl;
        ...
        s += endl + "}" + endl + "}\n";
        ;
        // s += "\t\tcomputeUtilities();"+ endl +"}" + endl ;
        File domainFileJava;
        File domainFileText;

        if (this.outputDirectory != null) {
            domainFileJava = new File(this.outputDirectory, name + ".java");
            domainFileText = new File(this.outputDirectory, name + ".txt");
        } else {
            domainFileJava = new File(name + ".java");
            domainFileText = new File(name + ".txt");
        }
        //-- Open the file with the appropriate name.
        BufferedWriter dest = new BufferedWriter(new FileWriter(domainFileJava));
        ...
        dest = new BufferedWriter(new FileWriter(domainFileText));
    ...
    public void commandInitialize() throws IOException {
        ...
        //-- Open the file.
        src = new BufferedReader(new FileReader(domainFileText));
    ...
    public void commandToCode(LinkedList states, LinkedList taskLists)
        ...
        s = "import utilities.jshop2.*;" + endl + endl;
        ...
        //-- JSHOP2 algorithm.
    /*AlgorithmInterface retAlgo = Factory.getInstance().getAlgorithm();
    s += endl + "\t\t"+retAlgo.getName() +".initialize(d, s);" + endl + endl;*/

        if (outputDirectory != null) {
            s += endl + String.format("\t\tFactory.getInstance().setOutputDirectory(\"%s\");",
                    outputDirectory.getPath().replace("\\", "\\\\")) + endl + endl;
        }

        s += endl + String.format("\t\tFactory.getInstance().setCSV(%s);", Factory.getInstance().isCSV()) + endl + endl;

        if (!Factory.getInstance().isJSHOP2()) {
            s += endl + "\t\tFactory.getInstance().setIsJSHOP2(false);" + endl + endl;
            s += endl + "\t\tFactory.getInstance().setResource(" + Factory.getInstance().getResource() + ");" + endl + endl;
        } else {
            s += endl + "\t\tFactory.getInstance().setIsJSHOP2(true);" + endl + endl;
        }

        s += endl + "\t\tFactory.getInstance().getAlgorithm().initialize(d, s);" + endl + endl;

        if (!Factory.getInstance().isJSHOP2()) {
            s += String.format("\t\tMyJSHOP2 myAlgo = MyJSHOP2.getInstance();%s%s", endl, endl);
            s += String.format("\t\tmyAlgo.setAlgorithmArgs(%s, %s);%s%s",Factory.getInstance().getResource(), Factory.getInstance().getRiskAttitude(), endl, endl);
            s += String.format("\t\tmyAlgo.findUtilities();%s%s", endl, endl);
        }
        ...
        BufferedWriter dest;
        File problemFileJava;

        if (this.outputDirectory != null) {
            problemFileJava = new File(this.outputDirectory, probName + ".java");
        } else {
            problemFileJava = new File(probName + ".java");
        }

        //-- Open the file with the appropriate name.
        dest = new BufferedWriter(new FileWriter(problemFileJava));
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (((args.length != 3) || planNo <= 0) && (args.length != 1) && (args.length != 5)) {
        ...
        if (args.length == 3) {
            ...
            File passedFile = new File(args[1]);
            InternalDomain domain = new InternalDomain(passedFile, planNo);

            if (passedFile.getParentFile() != null && passedFile.getParentFile().exists()) {
                domain.outputDirectory = passedFile.getParentFile();
            }

            if (args[2].equalsIgnoreCase("csv")) {
                Factory.getInstance().setCSV(true);
            }

            domain.parser.command();
        } else if (args.length == 5) {

            Factory.getInstance().setIsJSHOP2(false);
            Factory.getInstance().setResource(Double.parseDouble(args[2]));
            Factory.getInstance().setRiskAttitude(Integer.parseInt(args[3]));
            Factory.getInstance().setCSV(args[4].equalsIgnoreCase("csv"));
            File passedFile = new File(args[1]);
            InternalDomain domain = new InternalDomain(passedFile, planNo);
        ...
        //-- If this is a planning domain, call the 'domain' rule in the parser.
            ...
            InternalDomain domain = new InternalDomain(passedFile, -1);
